...
public class PostRepository {
	...
	public List<Post> findPostsByUsername(String aUsername) {
		...
		try {
			...
			return postsData.stream().filter(postData -> ((String) postData.get("posted_by")).equals(aUsername))
					.map(postData -> new Post((String) postData.get("text"))).collect(Collectors.toList());
		} catch (Exception e) {
